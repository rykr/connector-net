...
//		}
		...
		{
			int result = Query(mysql, queryPacket.Buffer, (uint)(queryPacket.Length-4));
			if (result != 0)
				throw new MySqlException(ErrorMsg(mysql), ErrorNumber(mysql));
		...
		public override IMySqlValue ReadColumnValue(int index, MySqlField field, IMySqlValue value)
		{
			...
			if (fieldPtr != IntPtr.Zero) 
			{
                ...
                p = new MySqlPacket(new MemoryStream(buf));
                p.Encoding = encoding;
			}
			
			return value.ReadValue(p, currentLengths[index], fieldPtr == IntPtr.Zero); 
		}
		...
		public override MySqlField[] ReadColumnMetadata(int count)
		{
			...
			for (int i=0; i < count; i++)
			{
                ...
                fields[i].SetTypeAndFlags(fieldDef.type, (ColumnFlags)fieldDef.flags);
